    ...
    UUPSUpgradeable
{
     ...
     */
    function _createPilotSessionsTable() private {
     ...
     */
    function _startTime(uint256 tokenId) private view returns (uint64) {
    ...
    function _canPilot(uint256 tokenId) private view returns (bool) {
        ...
        return
            ...
            (status == GarageStatus.TRAINING &&
                block.number >= _startTime(tokenId) + _PILOT_TRAINING_DURATION);
    }
    ...
    function pilotRig(
        ...
        if (_pilotStatus(tokenId) == GarageStatus.TRAINING) {
            ...
            string memory filters = string.concat(
                ...
                " and ",
                "start_time=",
                StringsUpgradeable.toString(_startTime(tokenId))
            );
            TablelandDeployments.get().runSQL(
                address(this),
                _pilotSessionsTableId,
    ...
    function parkRig(uint256 tokenId) public {
        ...
        uint64 startTime = _startTime(tokenId);
        if (
            status == GarageStatus.TRAINING &&
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            ...
            if (_startTime(tokenId) > 0) revert InvalidPilotStatus();
        }
